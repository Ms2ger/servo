...
impl PropertyAnimation {
    ...
    pub fn from_transition(transition_index: usize,
        ...
        let transition_property = new_style.as_servo().get_box().transition_property.0[transition_index];
        ...
        for transition_property in transition_property::computed_value::ALL_TRANSITION_PROPERTIES.iter() {
            if let Some(property_animation) = PropertyAnimation::from_transition_property(*transition_property,
                                                                                          transition_index,
                                                                                          old_style,
                                                                                          new_style) {
    ...
    pub fn update(&self, style: &mut ServoComputedValues, time: f64) {
        let progress = match self.timing_function {
            ...
            TransitionTimingFunction::Steps(steps, StartEnd::Start) => (time * (steps as f64)).ceil() / (steps as f64),
            TransitionTimingFunction::Steps(steps, StartEnd::End) => (time * (steps as f64)).floor() / (steps as f64),
        };
    ...
    where T: Interpolate,
{
    ...
    fn interpolate(&self, other: &Option<T>, time: f64) -> Option<Option<T>> {
        match (self, other) {
            (&Some(ref this), &Some(ref other)) => this.interpolate(other, time).and_then(|value| Some(Some(value))),
            (_, _) => None,
        }
    }
}
...
impl Interpolate for ZIndex {
    ...
    fn interpolate(&self, other: &ZIndex, time: f64) -> Option<ZIndex> {
        match (*self, *other) {
            (ZIndex::Number(ref this), ZIndex::Number(ref other)) => {
            ...
            (VerticalAlign::LengthOrPercentage(LengthOrPercentage::Length(ref this)),
             VerticalAlign::LengthOrPercentage(LengthOrPercentage::Length(ref other))) => {
                this.interpolate(other, time)
                    .and_then(|value| Some(VerticalAlign::LengthOrPercentage(LengthOrPercentage::Length(value))))
            },
            (_, _) => None,
        }
    }
...
impl Interpolate for CalcLengthOrPercentage {
    #[inline]
    fn interpolate(&self, other: &CalcLengthOrPercentage, time: f64) -> Option<CalcLengthOrPercentage> {
    ...
    fn interpolate(&self, other: &LengthOrPercentage, time: f64) -> Option<LengthOrPercentage> {
        match (*self, *other) {
            (LengthOrPercentage::Length(ref this), LengthOrPercentage::Length(ref other)) => {
                this.interpolate(other, time).and_then(|value| Some(LengthOrPercentage::Length(value)))
            },
            (LengthOrPercentage::Percentage(ref this), LengthOrPercentage::Percentage(ref other)) => {
                this.interpolate(other, time).and_then(|value| Some(LengthOrPercentage::Percentage(value)))
            },
            (this, other) => {
                let this: CalcLengthOrPercentage = From::from(this);
                let other: CalcLengthOrPercentage = From::from(other);
                this.interpolate(&other, time).and_then(|value| Some(LengthOrPercentage::Calc(value)))
            },
        }
    }
}
...
impl Interpolate for LengthOrPercentageOrAuto {
    #[inline]
    fn interpolate(&self, other: &LengthOrPercentageOrAuto, time: f64) -> Option<LengthOrPercentageOrAuto> {
        match (*self, *other) {
            (LengthOrPercentageOrAuto::Length(ref this), LengthOrPercentageOrAuto::Length(ref other)) => {
                this.interpolate(other, time).and_then(|value| Some(LengthOrPercentageOrAuto::Length(value)))
            },
            (LengthOrPercentageOrAuto::Percentage(ref this), LengthOrPercentageOrAuto::Percentage(ref other)) => {
                this.interpolate(other, time).and_then(|value| Some(LengthOrPercentageOrAuto::Percentage(value)))
            },
            (LengthOrPercentageOrAuto::Auto, LengthOrPercentageOrAuto::Auto) => Some(LengthOrPercentageOrAuto::Auto),
            (this, other) => {
...
impl Interpolate for LengthOrPercentageOrNone {
    #[inline]
    fn interpolate(&self, other: &LengthOrPercentageOrNone, time: f64) -> Option<LengthOrPercentageOrNone> {
        match (*self, *other) {
            (LengthOrPercentageOrNone::Length(ref this), LengthOrPercentageOrNone::Length(ref other)) => {
                this.interpolate(other, time).and_then(|value| Some(LengthOrPercentageOrNone::Length(value)))
            },
            (LengthOrPercentageOrNone::Percentage(ref this), LengthOrPercentageOrNone::Percentage(ref other)) => {
                this.interpolate(other, time).and_then(|value| Some(LengthOrPercentageOrNone::Percentage(value)))
            },
            (LengthOrPercentageOrNone::None, LengthOrPercentageOrNone::None) => Some(LengthOrPercentageOrNone::None),
            (_, _) => None,
        }
    }
}
...
impl Interpolate for LineHeight {
    ...
    fn interpolate(&self, other: &LineHeight, time: f64) -> Option<LineHeight> {
        match (*self, *other) {
            (LineHeight::Length(ref this), LineHeight::Length(ref other)) => {
                this.interpolate(other, time).and_then(|value| Some(LineHeight::Length(value)))
            },
            (LineHeight::Number(ref this), LineHeight::Number(ref other)) => {
                this.interpolate(other, time).and_then(|value| Some(LineHeight::Number(value)))
            },
            (LineHeight::Normal, LineHeight::Normal) => Some(LineHeight::Normal),
            (_, _) => None,
        }
    }
}
...
impl Interpolate for BackgroundPosition {
    ...
    fn interpolate(&self, other: &BackgroundPosition, time: f64) -> Option<BackgroundPosition> {
        match (self.horizontal.interpolate(&other.horizontal, time), self.vertical.interpolate(&other.vertical, time)) {
...
fn interpolate_transform_list(from_list: &[TransformOperation],
    ...
    if can_interpolate_list(from_list, to_list) {
        for (from, to) in from_list.iter().zip(to_list) {
            match (from, to) {
                (&TransformOperation::Matrix(from), &TransformOperation::Matrix(_to)) => {
                ...
                (&TransformOperation::Translate(fx, fy, fz), &TransformOperation::Translate(tx, ty, tz)) => {
                    let ix = fx.interpolate(&tx, time).unwrap();
                    let iy = fy.interpolate(&ty, time).unwrap();
                ...
                (&TransformOperation::Rotate(fx, fy, fz, fa), &TransformOperation::Rotate(_tx, _ty, _tz, _ta)) => {
                    // TODO(gw): Implement matrix decomposition and interpolation
                    result.push(TransformOperation::Rotate(fx, fy, fz, fa));
                },
                (&TransformOperation::Perspective(fd), &TransformOperation::Perspective(_td)) => {
                    // TODO(gw): Implement matrix decomposition and interpolation
...
pub fn start_transitions_if_applicable<C: ComputedValues>(new_animations_sender: &Mutex<Sender<Animation>>,
    ...
    for i in 0..new_style.get_box().transition_count() {
        ...
        for property_animation in property_animations {
            ...
            let start_time = now + (box_style.transition_delay.0.get_mod(i).seconds() as f64);
            new_animations_sender.lock()
                .unwrap()
                .send(Animation {
                    node: node,
                    property_animation: property_animation,
                    start_time: start_time,
                    end_time: start_time + (box_style.transition_duration.0.get_mod(i).seconds() as f64),
                })
                .unwrap();
