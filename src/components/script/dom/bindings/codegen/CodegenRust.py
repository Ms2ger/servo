        ...
        if allowTreatNonCallableAsNull and type.treatNonCallableAsNull():
            ...
            if not isDefinitelyObject:
                ...
                template = CGIfElseWrapper(haveObject,
                                           ...
                                           CGGeneric("${declName} = ptr::null();")).define()
            else:
                template = conversion
        else:
...
class CGTemplatedType(CGWrapper):
    def __init__(self, templateName, child, isReference=False):
        pre = "%s%s<" % ("&" if isReference else "", templateName)
        CGWrapper.__init__(self, child, pre=pre, post=">")
        ...
        if not name in unionStructs:
            ...
            for f in t.flatMemberTypes:
                ...
                if f.isInterface():
                    ...
                    # make our union.
                    headers.add(CGImports.getDeclarationFilename(f.inner))
                    ...
                    # code.
                    headers.add(CGImports.getDeclarationFilename(f))
...
class FakeArgument():
    ...
    """
    def __init__(self, type, interfaceMember, allowTreatNonCallableAsNull=False):
        self.type = type
        self.optional = False
        self.variadic = False
...
class CGNativeMember(ClassMethod):
    ...
    def getRetvalInfo(self, type, isMember):
        ...
        if type.isVoid():
            ...
            resultIsBoolean = type.tag() == IDLBuiltinType.Types.boolean
            defaultReturnArg = "0" if not resultIsBoolean else "false"
            if type.nullable():
                result = CGTemplatedType("Nullable", result)
                defaultReturnArg = ""
            typeDecl, errorDefault, template = \
                (result.define(),
                 "Ok(%s%s)" % (defaultReturnArg, (" as %s" % result.define()) if not resultIsBoolean else ""),
                 "return Ok(${declName});")
        elif type.isDOMString():
            if isMember:
            ...
            typeDecl, errorDefault, template = \
                "JSVal", "Ok(UndefinedValue())", "return Ok(${declName});"
        elif type.isObject():
            typeDecl, errorDefault, template = \
...
class CGCallbackFunctionImpl(CGGeneric):
    def __init__(self, callback):
        impl = """impl CallbackContainer for %s {
...
''').substitute({'name': name}))]
    ...
    def UnionTypes(config):
        ...
        curr = CGImports(curr, [
            ...
            'dom::bindings::conversions::{FromJSValConvertible, ToJSValConvertible, Default}',
